...
def when_close():
    #when user close the window temp data base answers.db
    #is removed from local disk
    print("removing temp files from the local disk")
...
def add_location_th01():
    x=0
    run_loc = create_tables()

    for key,value in run_loc:
        if x == 5:
            break
        else:
            x +=1
