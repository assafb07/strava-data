...
def all_shoes():
    ...
    text_box = Text(root, height=11, width=50, bg = text_box_bg, font=(fnt,fnt_size))
    for z,shoe in enumerate(all_shoes):
        gear_id = shoe[0]
...
def long_fast_year(i):
    year_runs = this_year()
    if i == 11:
        distance = lambda year_runs : year_runs[1]
        year_runs.sort(key=distance, reverse = True)
        long_fast = (year_runs[0])
    elif i == 12:
        pace = lambda year_runs : year_runs[2]
        year_runs.sort(key=pace, reverse = False)
        long_fast = (year_runs[0])

    longitude  = long_fast[6]
    latitude = long_fast[7]
    location = find_location(longitude,latitude)
    shoe = get_shoe_name(long_fast[5])
    ks = (long_fast[1])/1000
    pace = long_fast[2]
    atime = ks * pace
    print_time = make_time(atime)
    print_pace = make_time(long_fast[2])


    text_box = Text(root, height=11, width=50, bg = text_box_bg, font=(fnt,fnt_size))
    to_print = (f'{long_fast[1]}\n  Distance: {round(ks,3)}Km\n  Time: {print_time}\n  Pace: {print_pace}\n  Max HR: {long_fast[4]}\n  Avarage HR: {long_fast[3]}\n  Shoe: {shoe[0]} \n  Location: {location[0]}, {location[1]}, {location[2]}\n\n')
...
def load_frame():
    th = Thread(target=return_answers)
    th.start()
    ...
    activeforeground = afg, height = hi, width = wid, command=lambda:long_fast_year(12))
    ...
    activeforeground = afg, height = hi, width = wid, command=lambda:long_fast_year(11))
